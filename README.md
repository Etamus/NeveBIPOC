...
NeveBIPOC é uma aplicação desktop (Python + Tkinter) que gera, com um clique, um **dashboard analítico completo** renderizado em HTML usando a biblioteca [Plotly](https://plotly.com/python/). O dashboard é aberto automaticamente no navegador padrão e funciona de forma totalmente offline depois de carregado (exceto o mapa coroplético, que busca o GeoJSON via CDN).
